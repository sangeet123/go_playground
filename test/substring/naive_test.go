...
func TestIsASubString(t *testing.T) {
	text := "mangobanana"
	word := "banana"
	if i := substring.RobinKarp(text, word); i != 5 {
...
func TestIsNotASubString(t *testing.T) {
	text := "mangobanana"
	word := "bananaaaa"
	if i := substring.RobinKarp(text, word); i != -1 {
...
func TestTextAndWordOfEqualLength(t *testing.T) {
	text := "banana"
	word := "banana"
	if i := substring.RobinKarp(text, word); i != 0 {
		t.Error("expected ", -1, " but received ", i)
	}
}
...
func TestIsASubStringLongText(t *testing.T) {
	...
	if i := substring.RobinKarp(text, word); i != 19 {
