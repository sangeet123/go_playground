...
func createNeighbor(allWords []string, node string)[]string{
	neighbors := []string{}
	for i := 0; i < len(allWords); i++ {
		if diffBySingleChar(allWords[i], node){
		    neighbors = append(neighbors,allWords[i])
		}
	}
	return neighbors
}

func createSolution(source string, target string, neighborsMap map[string][]string)[][]string{
	result := new(Result)
	solveBFS(source, target, neighborsMap, result)
...
func solveBFS(source string, target string, neighborsMap map[string][]string, result *Result){
	...
	prepareResult(source, target, creator, result, temp_solution )
	//fmt.Println(creator)
}

func prepareResult(source string, target string, creator map[string][]string, result *Result, temp_solution[]string){

	if target == source{
		temp_solution = append([]string{target}, temp_solution...)
		tmp := make([]string, len(temp_solution))
		copy(tmp, temp_solution)
		result.ladder_list = append(result.ladder_list, tmp)
	}else{
		temp_solution = append([]string{target}, temp_solution...)
		for _, val := range creator[target]{
			prepareResult(source, val, creator, result, temp_solution )
		}
...
func findLadders(source string, target string, dict []string)[][]string{
	...
	for i,val := range allWords{
	    _, ok := neighborsMap[allWords[i]]
	    if !ok{
	        neighborsMap[val] = createNeighbor(allWords, val)            
	    }
	}
	return createSolution(source, target, neighborsMap)
}
