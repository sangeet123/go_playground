...
type Result struct {
	ladder_list StrMatrix
}

// update map by adding wordA and wordB as each other neighbor
func updateMap(neighborsMap map[string][]string, wordA string, wordB string) {
	neighborsA, _ := neighborsMap[wordA]
	neighborsMap[wordA] = append(neighborsA, wordB)
	neighborsB, _ := neighborsMap[wordB]
	neighborsMap[wordB] = append(neighborsB, wordA)
}

func createNeighbor(allWords []string) map[string][]string {
	neighborsMap := make(map[string][]string)
	for i := 0; i < len(allWords); i++ {
		for j := i + 1; j < len(allWords); j++ {
			if diffBySingleChar(allWords[i], allWords[j]) {
				updateMap(neighborsMap, allWords[i], allWords[j])
			}
		}
	}
	return neighborsMap
...
func isPresent(source string, dict []string) bool {
...
//checks if two words diff by single char
func diffBySingleChar(strA string, strB string) bool {
	...
	diffCount := 0
	for i := 0; i < len(strA) && diffCount < 2; i++ {
		if strA[i] != strB[i] {
			diffCount++
		}
	}
...
func getNeighbors(words []string, source string, neighbors map[string]bool) []string {
	unexplored_words := []string{}
	for _, word := range words {
		if word != source {
			if diffBySingleChar(word, source) {
				neighbors[word] = true
			} else {
				unexplored_words = append(unexplored_words, word)
			}
		}
	}
...
func create_solution(lvlot map[int]map[string]bool, depth int, result *Result, partial_solution []string) {

	if depth == -1 {
		result.ladder_list = append(result.ladder_list, partial_solution)
		return
	}

	for k, _ := range lvlot[depth] {
		if diffBySingleChar(k, partial_solution[0]) {
			partial_solution = append([]string{k}, partial_solution...)
			create_solution(lvlot, depth-1, result, partial_solution)
...
func findLadders(source string, target string, dict []string) [][]string {
	// if source is already equal to target return nil
	if source == target {
		return nil
	}

	// initialization before moving to level order traversal
	unexplored_words := dict
	target_found := false
	depth := 0
	level_traversal := make(map[int]map[string]bool)
	first_level := make(map[string]bool)
	first_level[source] = true
	level_traversal[depth] = first_level
	allWords = append([]string{source}, dict...)

	allWords := []string{}
	if !isPresent(source, dict) {
		allWords = append([]string{source}, dict...)
	} else {
		allWords = dict[:]
	}
	neighborsMap := createNeighbor(allWords)

	for !target_found && len(level_traversal[depth]) != 0 {
		next_level_words := make(map[string]bool)
		for word, _ := range level_traversal[depth] {
			unexplored_words = getNeighbors(unexplored_words, word, next_level_words)
			_, target_found = next_level_words[target]
		}

		if !target_found {
			depth++
			level_traversal[depth] = next_level_words
		}
	}

	if len(level_traversal[depth]) == 0 {
		return nil
	}

	result := new(Result)
	partial_solution := []string{target}
	// now we create a solution
	create_solution(level_traversal, depth, result, partial_solution)
	return result.ladder_list
}
